...
FACE_CDZS_EXPORT int FR_FacePTS(const STRU_ImgData_T &imagedata,const STRU_Rect_T &rect,STRU_PTS_T &pts);
/*
FR_FaceGlassesJudge说明：二维人脸戴眼镜judge
输入：imagedata 输入图片数据;  rect 人脸区域;  landmark  人脸五点;
输出：glassesinfo(0=不戴眼镜 1=戴眼镜 2=墨镜);
返回值：0表示成功 -1表示失败
*/
FACE_CDZS_EXPORT int FR_FaceEyeMouthClose(const STRU_ImgData_T &imagedata,const STRU_Rect_T &rect,int &eye_close,int &mouth_close);
/*
FR_FaceEyeClose说明：
输入：imagedata 输入图片数据;  rect 人脸区域;
输出：eye_close(0=close 1=open);
返回值：0表示成功 -1表示失败
*/
