...
int main()
{
    ...
    ifstream fin( "images_helmet.txt");
    ...
    while(fin>>image_name)
    {
        ...
        cv::Mat image;
        std::string img_path="./images_helmet/"+image_name;
//        std::cout << " Read from file: " << img_path <<std:: endl;
        ...
        for(int i=0;i<rectinfo.nFaceNum;i++)
        {
            ...
            int blurinfo;
            FR_FaceQualityBlurJudge(imagedata, rect, blurinfo);
            if (blurinfo == 1) {save_path = base_dir+"MASK/" + image_name;}
            if (blurinfo == 0) {save_path = base_dir+"NOMASK/" + image_name;}
        ...
        if (rectinfo.nFaceNum<1){
             ...
             std::cout << n<<"未检测的到人脸"<<save_path<< std::endl;
